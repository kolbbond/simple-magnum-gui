...
uniform float threshold = 1.0;      // in linear space
uniform float softKnee  = 0.5;      // [0..1], fraction of threshold
...
float lumaLinear(vec3 col) { 
	return dot(col, vec3(0.2126f, 0.7152f, 0.0722f)); 
}

float KarisAverage(vec3 colLinear) {
    float luma = lumaLinear(colLinear);
    return 1.0f / (1.0f + luma);
}
