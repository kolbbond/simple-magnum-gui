...
void Plotter::plot_scatter(std::string str, arma::Col<double> x,
                           arma::Col<double> y) {
  std::vector<double> xv = arma::conv_to<std::vector<double>>::from(x);
  std::vector<double> yv = arma::conv_to<std::vector<double>>::from(y);

  // ImPlot::PlotScatter("Points", &data[0].x, &data[0].y, data.size(), 0, 0, 2
  // * sizeof(double));
  ImPlot::PlotScatter(str.c_str(), xv.data(), yv.data(),
                      static_cast<int>(yv.size()));
}

// override for single point
void Plotter::plot_scatter(double x, double y) {
  plot_scatter(arma::Col<double>({x}), arma::Col<double>({y}));
}
...
// adds a line plot to whatever the current ImPlot context is
void Plotter::plot_line(std::string str, arma::Col<double> x,
                        arma::Col<double> y) {
  ...
  // add line
  ImPlot::PlotLine(str.c_str(), xv.data(), yv.data(), static_cast<int>(xv.size()));
}
...
void Plotter::set_axes_limits(arma::Col<double> x, arma::Col<double> y,
                              double xbuf, double ybuf) {
  // set manual limits
...
void Plotter::set_axes_limits(double xmin, double xmax, double ymin,
                              double ymax) {
  // this call does assumes only the first call i.e. (setup)
  //	ImPlot::SetupAxesLimits(minx, maxx, miny, maxy);
  // use imgui flags to force limits
...
void Plotter::set_axes_limits2(double xmin, double xmax, double ymin,
                               double ymax) {
  // this call does assumes only the first call i.e. (setup)
  //	ImPlot::SetupAxesLimits(minx, maxx, miny, maxy);
  // use imgui flags to force limits
...
/*
void Plotter::save_texture_as_png(Magnum::GL::Texture2D texture,
