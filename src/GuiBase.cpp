...
GuiBase::GuiBase(const Arguments &arguments)
  ...
  for (int i = 0; i < num_displays; i++) {
    ...
    _lg->msg("display %i bounds: %s(%i,%i)%s\n", i, KRED, displayBounds[i].x,
             displayBounds[i].y, KNRM);
  }
  ...
  std::vector<std::string> font_names = {
      "Roboto-Medium.ttf", "SourceSansPro-Regular.ttf", "DroidSans.ttf",
      "Cousine-Regular.ttf", "Karla-Regular.ttf"};
  ...
  for (size_t i = 0; i < font_names.size(); i++) {
    ...
    snprintf(font_cfg.Name, IM_ARRAYSIZE(font_cfg.Name), "%s, %0.1f px",
             font_names[i].c_str(), num_pixels);
    ImGui::GetIO().Fonts->AddFontFromMemoryTTF(
        const_cast<char *>(font.data()), static_cast<int>(font.size()),
